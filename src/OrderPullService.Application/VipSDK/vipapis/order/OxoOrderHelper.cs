...
namespace vipapis.order{
	...
	public class OxoOrderHelper : TBeanSerializer<OxoOrder>
	{
		...
		public void Read(OxoOrder structs, Protocol iprot){
			...
			if(schemeStruct != null){
				...
				while(true){
					...
					if ("barcodes".Equals(schemeField.Trim())){
						...
						while(true){
							...
							try{
								...
								vipapis.order.OxoOrderDetail elem0;
								
								elem0 = new vipapis.order.OxoOrderDetail();
								vipapis.order.OxoOrderDetailHelper.getInstance().Read(elem0, iprot);
					...
					if ("order_source".Equals(schemeField.Trim())){
						...
						while(true){
							...
							try{
								...
								vipapis.order.OxoOrderSource elem2;
								
								elem2 = new vipapis.order.OxoOrderSource();
								vipapis.order.OxoOrderSourceHelper.getInstance().Read(elem2, iprot);
					...
					if ("street_code".Equals(schemeField.Trim())){
						
						needSkip = false;
						string value;
						value = iprot.ReadString();
						
						structs.SetStreet_code(value);
					}
		...
		public void Write(OxoOrder structs, Protocol oprot){
			...
			if(structs.GetStreet_code() != null) {
				
				oprot.WriteFieldBegin("street_code");
				oprot.WriteString(structs.GetStreet_code());
