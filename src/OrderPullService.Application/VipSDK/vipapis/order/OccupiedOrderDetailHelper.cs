...
namespace vipapis.order{
	...
	public class OccupiedOrderDetailHelper : TBeanSerializer<OccupiedOrderDetail>
	{
		...
		public void Read(OccupiedOrderDetail structs, Protocol iprot){
			...
			if(schemeStruct != null){
				...
				while(true){
					...
					if ("is_area_sales".Equals(schemeField.Trim())){
						
						needSkip = false;
						bool value;
						value = iprot.ReadBool();
						
						structs.SetIs_area_sales(value);
					}
		...
		public void Write(OccupiedOrderDetail structs, Protocol oprot){
			...
			if(structs.GetIs_area_sales() != null) {
				
				oprot.WriteFieldBegin("is_area_sales");
				oprot.WriteBool((bool)structs.GetIs_area_sales());
				
				oprot.WriteFieldEnd();
			}
			
			else{
				throw new ArgumentException("is_area_sales can not be null!");
			}
