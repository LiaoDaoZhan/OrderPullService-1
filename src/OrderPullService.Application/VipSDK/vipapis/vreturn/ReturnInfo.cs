...
namespace vipapis.vreturn{
	...
	public class ReturnInfo {
		...
		///<summary>
		/// 退供类型：1，一退；2，二退；3，三退；20，VMI二退；21，部分二退；30，无PO退供；40 残次按PO退供；100，VIS二退（VIS使用）；101，VIS普通退供（VIS使用）；102，VIS残次退供（VIS使用）；103，VIS VIP_3PL退品骏普通退供（VIS使用）；104，VIS VIP_3PL退品骏残次退供（VIS使用）；105，VIS VIP_3PL退供应商（VIS使用）；106，VIS一退退供（VIS使用）；110，门户正常退供（PSP使用）；111，门户残次退供（PSP使用）；112，门户过期退供（PSP使用）；113，门户入库凭证退供（PSP使用）
