...
namespace vipapis.puma{
	...
	public class PriceHelper : TBeanSerializer<Price>
	{
		...
		public void Read(Price structs, Protocol iprot){
			...
			if(schemeStruct != null){
				...
				while(true){
					...
					if ("special_price".Equals(schemeField.Trim())){
						
						needSkip = false;
						string value;
						value = iprot.ReadString();
						
						structs.SetSpecial_price(value);
					}
		...
		public void Write(Price structs, Protocol oprot){
			...
			if(structs.GetSpecial_price() != null) {
				
				oprot.WriteFieldBegin("special_price");
				oprot.WriteString(structs.GetSpecial_price());
