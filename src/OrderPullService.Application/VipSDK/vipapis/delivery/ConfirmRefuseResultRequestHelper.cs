...
namespace vipapis.delivery{
	...
	public class ConfirmRefuseResultRequestHelper : TBeanSerializer<ConfirmRefuseResultRequest>
	{
		
		public static ConfirmRefuseResultRequestHelper OBJ = new ConfirmRefuseResultRequestHelper();
		...
		public void Read(ConfirmRefuseResultRequest structs, Protocol iprot){
			...
			if(schemeStruct != null){
				...
				while(true){
					...
					if ("goods_list".Equals(schemeField.Trim())){
						...
						needSkip = false;
						List<vipapis.delivery.RefuseGoods> value;
						
						value = new List<vipapis.delivery.RefuseGoods>();
						iprot.ReadListBegin();
						while(true){
							
							try{
								
								vipapis.delivery.RefuseGoods elem1;
								
								elem1 = new vipapis.delivery.RefuseGoods();
								vipapis.delivery.RefuseGoodsHelper.getInstance().Read(elem1, iprot);
		...
		public void Write(ConfirmRefuseResultRequest structs, Protocol oprot){
			...
			if(structs.GetGoods_list() != null) {
				...
				foreach(vipapis.delivery.RefuseGoods _item0 in structs.GetGoods_list()){
					
					
					vipapis.delivery.RefuseGoodsHelper.getInstance().Write(_item0, oprot);
		...
		public void Validate(ConfirmRefuseResultRequest bean){
