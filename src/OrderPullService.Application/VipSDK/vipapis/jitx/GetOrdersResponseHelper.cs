	...
	public class GetOrdersResponseHelper : TBeanSerializer<GetOrdersResponse>
	{
			...
			if(schemeStruct != null){
				...
				while(true){
					...
					if ("orders".Equals(schemeField.Trim())){
						...
						needSkip = false;
						List<vipapis.jitx.Order> value;
						
						value = new List<vipapis.jitx.Order>();
						iprot.ReadListBegin();
						while(true){
							
							try{
								
								vipapis.jitx.Order elem1;
								
								elem1 = new vipapis.jitx.Order();
								vipapis.jitx.OrderHelper.getInstance().Read(elem1, iprot);
		...
		public void Write(GetOrdersResponse structs, Protocol oprot){
			...
			if(structs.GetOrders() != null) {
				...
				foreach(vipapis.jitx.Order _item0 in structs.GetOrders()){
					
					
					vipapis.jitx.OrderHelper.getInstance().Write(_item0, oprot);
