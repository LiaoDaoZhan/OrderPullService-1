...
namespace com.vip.svip.osp.service{
	...
	public class BuyLimitStateHelper : TBeanSerializer<BuyLimitState>
	{
		...
		public void Read(BuyLimitState structs, Protocol iprot){
			...
			if(schemeStruct != null){
				...
				while(true){
					...
					if ("canTry".Equals(schemeField.Trim())){
						
						needSkip = false;
						bool value;
						value = iprot.ReadBool();
						
						structs.SetCanTry(value);
					}
		...
		public void Write(BuyLimitState structs, Protocol oprot){
			...
			if(structs.GetCanTry() != null) {
				
				oprot.WriteFieldBegin("canTry");
				oprot.WriteBool((bool)structs.GetCanTry());
				
				oprot.WriteFieldEnd();
			}
			
			else{
				throw new ArgumentException("canTry can not be null!");
			}
